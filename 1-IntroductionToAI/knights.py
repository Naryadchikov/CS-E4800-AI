...
class KnightState:
    ...
    def successors(self):
        ...
        for initialState in self.occupied:
            occupiedCopy = self.occupied.copy()
            occupiedCopy.remove(initialState)
            ...
            for i in range(0, 8):
                x = initialState[0] + X[i]
                y = initialState[1] + Y[i]
                possibleKnightState = (x, y, initialState[2])

                if x >= 0 and y >= 0 and x < 8 and y < 8 and possibleKnightState not in self.occupied:
                    occupiedCopy.append(possibleKnightState)
                    successorsArray.append(
                        (str(X[i]) + "&" + str(Y[i]), KnightState(occupiedCopy.copy())))
                    occupiedCopy.remove(possibleKnightState)
