...
def value_of(mdp, s, a, v, gamma):
    ...
    for s2 in mdp.successor_states(s, a):
        value += mdp.P(s, a, s2) * (mdp.R(s, a, s2) + gamma * v[s2])
    ...
    while (True):
        for s in mdp.states():
            ...
            for a in mdp.applicable_actions(s):
                value = value_of(mdp, s, a, previosV, gamma)
        ...
        for s in mdp.states():
            if abs(v[s] - previosV[s]) >= 0.5 * epsilon * (1.0 - gamma) / gamma:
                shouldStop = False
                break
...
if __name__ == "__main__":
    ...
    for s in sorted(gdp.states()):
        print("Location: {0} \t | Value: {1} \t | Policy: {2}".format(
            s, v[s], pi[s]))
    print(
        """
