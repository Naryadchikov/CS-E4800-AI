...
def atMost1(fmas):
    return AND([NOT(AND([pair[0], pair[1]])) for pair in allpairs(fmas)])
...
def sudoku2fma(puzzle):
    ...
    C1 = AND([exactly1([S(c, r, n) for n in range(1, 10)])
              for r in range(1, 10) for c in range(1, 10)])

    # Every row has all numbers
    C2 = AND([exactly1([S(c, r, n) for c in range(1, 10)])
              for r in range(1, 10) for n in range(1, 10)])

    # Every column has all numbers
    C3 = AND([exactly1([S(c, r, n) for r in range(1, 10)])
              for c in range(1, 10) for n in range(1, 10)])

    # Every 3X3 sub-grid has all numbers
    C4 = AND([exactly1([S(3 * j + _c, 3 * i + _r, n) for _c in range(1, 4) for _r in range(1, 4)])
              for i in range(0, 3) for j in range(0, 3) for n in range(1, 10)])
