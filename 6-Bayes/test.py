...
def task_conditional_probability(data):
    ...
    for query, conditions, fields, answer in tests:
        ...
        if is_answer_close(prob, answer):
            print("correct probability P({}|{}) = {}".format(
                fields[0], " & ".join(fields[1:]), round(prob, 3)))
        else:
            print("Conditional probability failed: got {}, true answer {}".format(
                prob, answer))


# See that constructed CPT matches the example
def task_cpt(data):
    tests = [(SAIMAASEAL, [MILSERVICE, SCHOOLHYMN], ["SAIMAASEAL", "MILSERVICE",
                                                     "SCHOOLHYMN"], {"0 0": 0.587, "0 1": 0.790, "1 0": 0.834, "1 1": 0.857}), ]

    for query, conditions, fields, answer in tests:
        table = construct_probability_table(data, query, conditions)

        print("Calculating CPT for P({}|{})".format(
        ...
        for key, probability in table.items():
            ...
            passed = "Correct" if is_answer_close(
                answer[str_assignment], probability) else "Not right probability, correct: {}".format(answer[str_assignment])
            print("{} : {} <- {}".format(str_assignment,
                                         round(probability, 3), passed))


def test_brute_force(data, network):
    tests = [([MILSERVICE], ([MEDICALSERV, SAIMAASEAL, METROPOLIS], [0, 0, 0]), [
              "MILSERVICE", "MEDICALSERV", "SAIMAASEAL", "METROPOLIS"], 0.183)]
    ...
    for query, (E, e), fields, answer in tests:
        ...
        if is_answer_close(answer, prob):
            ...
            print("Wrong, true answer was {} while yours was {}".format(
                answer, round(prob, 3)))
...
def test_sampling(data, network):
    ...
    for query, (E, e), fields, answer in tests:
        prob = [approximate_distribution(
            network, query, E, e) for _ in range(3)]
        print("Calculating P({}|{})".format(fields[0], " & ".join(fields[1:])))
        ...
        if any([is_answer_close(answer, p, EPSILON=3e-2) for p in prob]):
            ...
            print("Wrong, true answer was {} while yours was {}".format(
                answer, round(np.average(prob), 3)))


def main():
    # Load data
    filename = "I:\\Uni\\Aalto\\ArtificialIntelligence\\PythonAssignments\\6-Bayes\\data\\hs.txt"
