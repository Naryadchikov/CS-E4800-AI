...
class bayes_network:
    ...
    def get_node_probability(self, feature, conditions):
        ...
        assert len(tmp) == len(
            Node.parents), "Not all parents of a node have a value"

        # Make a key
        dict_key = "vals"  # + str(B)
        dict_key += "".join([".{}:{}.".format(key, tmp[key])
                             for key in sorted(tmp.keys())])
        return Node.probabilities[dict_key]

    def append_node(self, node_id, name, parents=[]):
        Node = node(name, node_id)
        Node.probabilities = construct_probability_table(
            self.data, node_id, parents)
