...
def construct_probability_table(data, query, conditions):
    ...
    for assignment in assignments:
        ...
        dict_key = "vals"
        dict_key += "".join([".{}:{}.".format(key, B[key])
...
def exhaustive_enum(G, X, E, e):
    ...
    # Look for: Exhaustive Enumeration in chapter 6.4
    for assignment in assignments:
        ...
        for node in G.nodes:
            # =========================================================
            # TASK 2.1: From exhaustive search in chapter 6.4
            ...
            #
            #  HINTS:
            #   - See exhaustive search in chapter 6.4
            #   - combined dictionary consists of (node:binary) assignments
            # =========================================================
            tmp *= evaluate_node(G, node.data_id,
                                 combined_dictionary[node.data_id], combined_dictionary)
...
def brute_force(G, targets, E, e):
    ...
    pos = exhaustive_enum(G, X, E + targets, e +
                          [1 for _ in range(len(targets))])
    neg = exhaustive_enum(G, X, E + targets, e +
                          [0 for _ in range(len(targets))])
...
def sample(G, X, E, e):
    ...
    # Algorithm Sample(..) in chapter 6.4
    for variable in X:
        parents = dict(zip(variables, assignment))
        # =========================================================
        # TASK 2.2: complete the algorithm as in algorithm SAMPLE(..) on chapter 6.4
        #
        ...
        if variable in E:
            ...
            w *= evaluate_node(G, variable, value, parents)
        else:
